...
fn blit_opaque(
    ...
    let destination_x_start =
        destination_x.max(0) as usize;

    let destination_y_start =
        destination_y.max(0) as usize;
    ...
    let copy_width =
        (texture.width - source_x_start).min(
            destination_width - destination_x_start,
        );

    // Clip against the bottom side of the framebuffer.
    let copy_height =
        (texture.height - source_y_start).min(
            destination_height - destination_y_start,
        );
    ...
    for row in 0..copy_height {
        let source_start =
            (source_y_start + row) * texture.width
                + source_x_start;

        let destination_start =
            (destination_y_start + row)
                * destination_width
                + destination_x_start;

        let source_end =
            source_start + copy_width;

        let destination_end =
            destination_start + copy_width;
...
> {
    pub fbuf:
        &'a mut embedded_graphics_framebuf::FrameBuf<
            Rgb565,
            B,
        >,
}
    ...
    > DrawTarget for SpriteBuf<'a, B>
{
    ...
    {
        let width =
            self.fbuf.width() as i32;

        let height =
            self.fbuf.height() as i32;
...
pub fn render_system(
    ...
    let fast_textures =
        fast_textures.get_or_insert_with(|| {
            FastMazeTextures::from_assets(
                &texture_assets,
            )
        });
    ...
    let (
        ...
        _maze_right,
        _maze_top,
    ) = maze.playable_bounds();

    let tile_w =
        maze.tile_width as i32;

    let tile_h =
        maze.tile_height as i32;

    let display_width =
        crate::LCD_H_RES as i32;

    let display_height =
        crate::LCD_V_RES as i32;
    ...
    let min_tx =
        ((visible_left - maze_left) / tile_w)
            .max(0);

    let max_tx =
        ((visible_right - maze_left) / tile_w)
            .min(maze.width as i32 - 1);

    let min_ty =
        ((visible_bottom - maze_bottom) / tile_h)
            .max(0);

    let max_ty =
        ((visible_top - maze_bottom) / tile_h)
            .min(maze.height as i32 - 1);
    ...
    if min_tx <= max_tx && min_ty <= max_ty {
        ...
        for ty in min_ty..=max_ty {
            ...
            let maze_row_start =
                ty as usize * maze.width as usize;
            ...
            for tx in min_tx..=max_tx {
                ...
                let tile_type =
                    maze.data[tile_index];

                if let Some(texture) =
                    fast_textures.get(tile_type)
                {
                    blit_opaque(
                        framebuffer_pixels,
                        crate::LCD_H_RES,
...
fn draw_touch_button<B>(
    frame_buf:
        &mut embedded_graphics_framebuf::FrameBuf<
            Rgb565,
            B,
        >,
    rect: ButtonRect,
    ...
    >,
{
    ...
    let label_x =
        rect.x
            + (rect.width as i32 - 10) / 2;

    let label_y =
        rect.y
            + (rect.height as i32 + 20) / 2;
