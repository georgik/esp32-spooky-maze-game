...
use esp_hal::{
    ...
    main,
    mipi_dsi::{
        Config as MipiConfig, DataLanes, MipiDsi,
...
use spooky_core::{
    events::{
        coin::CoinCollisionMessage, dynamite::DynamiteCollisionMessage, npc::NpcCollisionMessage,
        player::PlayerInputMessage, walker::WalkerCollisionMessage,
    },
    resources::MazeSeed,
    systems::{self, collisions, hud::HudState, process_player_input::process_player_input},
};
...
use crate::{embedded_systems::render::render_system, heapbuffer::HeapBuffer};

use crate::touch::{
    Gt911, TouchEvent, TouchInputState, direction_at, dispatch_touch_input, transform_touch_point,
};
...
const MIPI_FB_SIZE: usize = LCD_BUFFER_SIZE * BYTES_PER_PIXEL;
...
fn elapsed_time() -> core::time::Duration {
    let microseconds = HalInstant::now().duration_since_epoch().as_micros();
...
fn allocate_mipi_framebuffer() -> &'static mut [u8] {
    let layout =
        Layout::from_size_align(MIPI_FB_SIZE, 64).expect("Invalid MIPI framebuffer layout");

    let pointer = unsafe { alloc::alloc::alloc_zeroed(layout) };

    assert!(!pointer.is_null(), "MIPI framebuffer allocation failed");

    unsafe { core::slice::from_raw_parts_mut(pointer, MIPI_FB_SIZE) }
}
...
fn main() -> ! {
    ...
    let mut lcd_reset = Output::new(peripherals.GPIO27, Level::Low, OutputConfig::default());
    ...
    let _lcd_backlight = Output::new(peripherals.GPIO26, Level::High, OutputConfig::default());
    ...
    let mipi_framebuffers: [&mut [u8]; 2] = [framebuffer_1, framebuffer_2];
    ...
    loop {
        ...
        match touch_controller.0.poll_event() {
            Ok(TouchEvent::Point(point)) => {
                ...
                let direction = direction_at(screen_x, screen_y);
            ...
            Err(error) => {
                println!("GT911 polling error: {:?}", error,);
            }
        }
        ...
        {
            let software_framebuffer = app.world().resource::<FrameBufferResource>();
            ...
            let destination_pixels: &mut [u16] = unsafe {
                core::slice::from_raw_parts_mut(
                    mipi_back_buffer.as_mut_ptr().cast::<u16>(),
                    LCD_BUFFER_SIZE,
                )
            };
            ...
            for (destination, source) in destination_pixels
                .iter_mut()
                .zip(software_framebuffer.frame_buf.data.iter())
            {
